...
def extracted_block_text(blocks: List[Dict[str, Any]]):
    """
    Возвращает список текстов для ВСЕХ блоков.
    Если текста нет - возвращает заглушку.
    """
    block_text = []
    for obj in blocks:
        if obj and "data" in obj and "text" in obj["data"] and obj["data"]["text"]:
            block_text.append(obj["data"]["text"])
        else:
            block_text.append("emptyText")
...
class MlClassifier(BaseClassifier):
    ...
    def classify(self, blocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        all_texts = extracted_block_text(blocks)
        ...
        for index, block in enumerate(blocks):
            ...
            classifed_block["classification_result"] = self.classify_three_blocks(previous_block, current_block, next_block, index - 1).label
    ...
    def classify_three_blocks(
        ...
        if not self.extract_text(current_block):
            label = "empty"
            confidence = 0.0
            normalise_block = {}
        else:
    ...
    def train(self, blocks: List[Dict[str, Any]]):
        train_block_text = extracted_block_text(blocks)
